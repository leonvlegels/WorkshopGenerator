...
export default async function WorkshopDetail({ params }: { params: { id: string } }) {
  const workshop = await prisma.workshopStructure.findUnique({
    where: { id: params.id },
    include: { workshopTemplate: true }
  });
  const [manuals, slides] = await Promise.all([
    prisma.manualArtifact.findMany({ where: { workshop_structure_id: params.id }, orderBy: { id: 'desc' } }),
    prisma.slideArtifact.findMany({ where: { workshop_structure_id: params.id }, orderBy: { id: 'desc' } })
  ]);
  ...
  return (
    <main style={{ padding: 24, maxWidth: 1000, margin: '0 auto' }}>
      ...
      <StructureEditor workshopId={workshop.id} initialModules={sequence.modules} />
      <ol>
      ...
      <pre style={{ whiteSpace: 'pre-wrap', background: '#fff', padding: 12 }}>{manuals[0]?.markdown_body}</pre>
      {manuals[0] && (
        <p>
          <a href={`/api/manuals/${manuals[0].id}/docx`}>Download DOCX</a>
        </p>
      )}
      <h2>Slides preview</h2>
      <pre style={{ whiteSpace: 'pre-wrap', background: '#fff', padding: 12 }}>{slides[0]?.preview_markdown}</pre>
      <p>
